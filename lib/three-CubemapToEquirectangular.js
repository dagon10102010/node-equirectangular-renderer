// this script is taken from Jaume Sanchez' https://github.com/spite/THREE.CubemapToEquirectangular
...
function CubemapToEquirectangular( renderer, provideCubeCamera, options ) {
	...
	this.canvas = options.canvas || this.createCanvas(options.width || 4096, options.height || 2048);
	//this.ctx = this.canvas.getContext( '2d' );

	this.cubeCamera = null;
	this.attachedCamera = null;

	this.setSize( options.width || 4096, options.height || 2048 );

	var gl = this.renderer.getContext();
	this.cubeMapSize = gl.getParameter( gl.MAX_CUBE_MAP_TEXTURE_SIZE )

	if( provideCubeCamera ) {
		this.getCubeCamera( options.height || 2048 )
	}
}

CubemapToEquirectangular.prototype.createCanvas = function( width, height ) {
	if (this.verbose) console.log('[CubemapToEquirectangular.createCanvas]');

	if (process.browser) {
		if (this.verbose) console.log('[CubemapToEquirectangular.createCanvas] browser detected');
	...
	if (this.verbose) console.log('[CubemapToEquirectangular.createCanvas] nodejs detected');
...
CubemapToEquirectangular.prototype.update = function( camera, scene, download ) {
	...
	this.convert( this.cubeCamera, download );

}

CubemapToEquirectangular.prototype.updateAndGetCanvas = function( camera, scene ) {
	// like update method
	var autoClear = this.renderer.autoClear;
	this.renderer.autoClear = true;
	this.cubeCamera.position.copy( camera.position );
	...
	try {
		imageData = new ImageData( new Uint8ClampedArray( pixels ), this.width, this.height );
	} catch( e ) {
		// when running server-side (node environment) ImageData API might not be available.
		// If so, recycle our canvas' internal imageData instance
		imageData = this.canvas.getContext('2d').createImageData( this.width, this.height );
